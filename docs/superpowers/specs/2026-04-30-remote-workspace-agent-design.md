...
```json
{
  ...
  "revision": "rev-...",
  "pulled": false,
  "pull_command": "xxx pull host:/workspace/checkpoints/model.tar.gz"
}
```
...
`remoted` runs on each remote server. It is a lightweight daemon, not a full Agent. It owns remote workspace inspection, file transfer, safe writes, command execution, PTY shell sessions, and file events.
...
`xxx` runs locally. It is the interface used by humans, scripts, and Agents.
...
- `xxx diff` shows local changes against the last synced base.
- `xxx apply` writes changes to the remote daemon after conflict checks.
- `xxx sync` never silently overwrites dirty local files.
...
```text
~/.xxx/state/<host-id>/<workspace-id>/
  manifest.sqlite
  base/
  patches/
  transfer-cache/
...
```bash
xxx host add lab-a --url http://10.0.0.12:7731
xxx workspace add lab-a:/data/project --local ~/remote/lab-a/project
xxx status lab-a:/data/project
```

### Sync

```bash
xxx sync lab-a:/data/project
xxx sync --watch lab-a:/data/project
xxx sync --force lab-a:/data/project
xxx sync --quiet lab-a:/data/project
xxx sync --dry-run lab-a:/data/project
```
...
```bash
xxx pull lab-a:/data/project/checkpoints/model.tar.gz
xxx pull lab-a:/data/project/checkpoints/
xxx pull --include-large lab-a:/data/project/checkpoints/
xxx pull --force lab-a:/data/project/checkpoints/
xxx pull --quiet lab-a:/data/project/checkpoints/
```
...
```bash
xxx diff lab-a:/data/project
xxx diff lab-a:/data/project/src/main.py
```
...
```bash
xxx apply lab-a:/data/project
xxx apply --dry-run lab-a:/data/project
xxx apply --force lab-a:/data/project
```
...
```bash
xxx restore lab-a:/data/project/src/main.py
xxx restore --all lab-a:/data/project
```
...
```bash
xxx exec lab-a:/data/project -- pytest -q
xxx exec --remote-only lab-a:/data/project -- nvidia-smi
xxx exec --no-sync-check lab-a:/data/project -- df -h
```
...
```bash
xxx shell lab-a:/data/project
xxx shell --remote-only lab-a:/data/project
xxx shell --list lab-a
xxx shell --attach <session-id>
```
...
1. Run `xxx pull` to materialize the file locally, or create a new local large file.
...
- HTTP framework and language for `remoted`.
